export const EMBED_IMAGE_SIZE = "1600x420";

// Replace these with final artwork. Keep the same 1600x420-ish ratio so embeds
// stay visually steady across pages.
export const EMBED_IMAGES = {
  help: {
  ...
  track: {
    ...
    note: "Could be transparent. If visible, make it a calm weekly recap/watchlist banner.",
  },
  tags: {
...
export type EmbedImageName = keyof typeof EMBED_IMAGES;

export function embedImage(name: EmbedImageName) {
  return {
    url: EMBED_IMAGES[name].url,
  };
}

export function quoteList(lines: Array<string | undefined | false>) {
  return lines
...
export function quoteLines(lines: Array<string | undefined | false>) {
  return lines
    .filter((line): line is string => Boolean(line))
    .map((line) => `> ${line}`)
    .join("\n");
...
export function pageFooter(page: number, pages: number, note?: string) {
  return {
