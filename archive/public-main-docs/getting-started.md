...
icon: rocket
order: 95
description: Your first few minutes with Patch.
...
+++ I want one clean picture
Use `/profile`.

Patch sends a PNG profile card that is easy to share in chat.
+++ I want the numbers
Use `/stats`.

Patch opens a three-page read: overview, current season, and public history.
+++ I want to compare two players
Use `/compare`.

Patch checks current-season ranked MMR, K/D, KDA, win rate, and kills per match.
+++ I want staff to review something
Use `/report`.

Bring image or video proof. Patch opens a short form so staff get the proof plus your explanation.
+++

!!!tip Tiny habit
If Patch cannot find someone, try the player ID. It is the name tag that does not change clothes.
!!!
